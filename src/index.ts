/* stylelens — is what you are looking at actually on the design system?
 *
 ...
 * re-exported the overlay, so `import 'stylelens'` needed React even to count
 * off-system values, while package.json called React optional.
 *
 *   window.__styleLens         a JSON API a coding agent calls
 *   new Lens().audit()      a whole-page count for CI
 *
 * The overlay a designer hovers is `stylelens/react`, which is the only entry
 * that imports React.
 *
...
export { createGlobal, type StyleLensGlobal } from './agent'
...
export type { Audit, StyleLensConfig, Inspection, Origin, Reading, Verdict } from './types'
